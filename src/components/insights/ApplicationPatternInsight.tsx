...
export function ApplicationPatternInsight() {
  ...
  if (isLoading) return <p style={{ fontSize: 13, color: warm.colors.textMuted }}>Computing your pattern…</p>;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      <div style={{ padding: 18, borderRadius: 12, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.18)' }}>
        <p style={{ margin: '0 0 12px', fontSize: 11, fontWeight: 800, letterSpacing: '0.12em', color: warm.colors.accentPetrol, textTransform: 'uppercase' }}>
          You're applying to
        </p>
        {data.appliedTo.length === 0 ? (
          <p style={{ margin: 0, fontSize: 13, color: warm.colors.textMuted }}>No applications yet.</p>
        ) : (
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.appliedTo.slice(0, 5).map((b, i) => (
              <li key={i} style={{ fontSize: 13, color: warm.colors.borderWhisper }}>
                <strong>{b.title}</strong>{' '}
                <span style={{ color: warm.colors.textMuted }}>
      ...
      <div style={{ padding: 18, borderRadius: 12, background: 'rgba(197,160,89,0.06)', border: '1px solid rgba(197,160,89,0.18)' }}>
        <p style={{ margin: '0 0 12px', fontSize: 11, fontWeight: 800, letterSpacing: '0.12em', color: warm.colors.accentGold, textTransform: 'uppercase' }}>
          Your resume is competitive for
        </p>
        {data.competitiveFor.length === 0 ? (
          fallback ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <p style={{ margin: 0, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: warm.colors.accentGold, textTransform: 'uppercase', opacity: 0.8 }}>
                From your diagnostic
              </p>
              <p style={{ margin: 0, fontSize: 14, color: warm.colors.borderWhisper, fontWeight: 600, lineHeight: 1.4, textTransform: 'capitalize' }}>
                {fallback.headline}
              </p>
              {fallback.positioning && (
                <p style={{ margin: 0, fontSize: 12, color: warm.colors.textMuted, lineHeight: 1.55, fontStyle: 'italic' }}>
                  {fallback.positioning}
                </p>
              )}
              <p style={{ margin: '4px 0 0', fontSize: 11, color: warm.colors.textMuted, lineHeight: 1.55 }}>
                Live job matches will appear here once the feed builds up.
              </p>
            </div>
          ) : (
            <p style={{ margin: 0, fontSize: 13, color: warm.colors.textMuted }}>Browse the Job Feed for a few days so we can compute this.</p>
          )
        ) : (
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.competitiveFor.map((j, i) => (
              <li key={i} style={{ fontSize: 13, color: warm.colors.borderWhisper }}>
                ...
                <span style={{ color: warm.colors.accentGold }}>{Math.round(j.matchScore)}%</span>
              </li>
            ))}
          </ul>
        )}
