...
function BarChart({ data, colour = S.petrol }: { data: { date: string; count: number }[]; colour?: string }) {
        ...
        {data.map((d, i) => {
          ...
          return (
            <rect key={d.date} x={x} y={y} width={barW} height={h}
              fill={d.count > 0 ? colour : warm.colors.borderWhisper}
...
function StatCard({
  label, value, sub, colour = S.petrol, icon: Icon,
}: {
  label: string; value: string | number; sub?: string; colour?: string; icon: React.FC<any>;
}) {
  return (
    <div style={{
      background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14,
...
function HBar({ label, value, total, colour }: { label: string; value: number; total: number; colour: string }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ height: 5, borderRadius: 99, background: warm.colors.bgAlt, overflow: 'hidden' }}>
...
function InsightsSection() {
  ...
  const accentByIndex = [S.petrol, S.gold, S.petrolLight, S.petrol, S.success];

  return (
    <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '20px 22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: insights.length ? 20 : 14, flexWrap: 'wrap', gap: 10 }}>
        ...
        <button onClick={generate} disabled={loading} style={{ ...btnStyle(S.petrol), opacity: loading ? 0.6 : 1, minWidth: 160, justifyContent: 'center' }}>
      ...
      {loading && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '20px 0', color: S.dim, fontSize: 13 }}>
          <div style={{ width: 16, height: 16, border: `2px solid ${S.petrol}40`, borderTopColor: S.petrol, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          Reading your data...
        </div>
      )}

      {error && <p style={{ color: S.danger, fontSize: 13, margin: 0 }}>{error}</p>}
...
function OverviewTab({ stats }: { stats: Stats }) {
  ...
  const statusColours: Record<string, string> = {
    SAVED: S.dim, APPLIED: S.petrol, INTERVIEW: S.gold, OFFER: S.success, REJECTED: S.danger,
  };
  const planColours: Record<string, string> = {
    free: S.dim, monthly: S.petrol, three_month: S.gold, annual: S.petrolLight,
  };
  const typeColours: Record<string, string> = {
    RESUME: S.success, COVER_LETTER: S.petrol, STAR_RESPONSE: S.petrolLight,
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
        ...
        <BarChart data={stats.users.daily} colour={S.petrol} />
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          ...
          <StatCard label="Diagnostics" value={stats.diagnostics.complete} sub={`${fmt(stats.diagnostics.thisWeek)} this week`} icon={ClipboardList} colour={S.gold} />
        </div>
      </div>
      ...
      <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
        ...
        <BarChart data={stats.generations.daily} colour={S.gold} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        ...
        <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
          ...
          {stats.feedback.avgRating != null && (
            <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${warm.colors.borderWhisper}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Star size={12} style={{ color: S.gold }} />
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 32, fontWeight: 900, color: S.petrol, lineHeight: 1 }}>{data.reportCount}</p>
        </div>
      ...
      {data.script ? (
        <>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            <button onClick={handleCopy} style={btnStyle(copied ? S.success : undefined)}>
              {copied ? <><Check size={13} /> Copied</> : <><Copy size={13} /> Copy script</>}
            </button>
            <button onClick={() => emailMutation.mutate()} disabled={emailMutation.isPending} style={btnStyle(S.petrol)}>
              <Mail size={13} />
          ...
          <div style={{
            ...
            whiteSpace: 'pre-wrap', fontSize: 15, lineHeight: 1.8, color: S.sub,
        ...
        <div style={{
          background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 16,
...
function btnStyle(accent?: string): React.CSSProperties {
  return {
    ...
    background: accent ? `${accent}18` : warm.colors.bgAlt,
    border: `1px solid ${accent ? `${accent}40` : warm.colors.borderWhisper}`,
    color: accent ?? S.sub,
...
function FunnelTab({ stats }: { stats: Stats }) {
  const stages = [
    { label: 'Accounts created',       value: stats.users.total,            colour: S.petrol,   note: 'Everyone who signed up' },
    { label: 'Completed onboarding',   value: stats.users.onboarded,        colour: S.petrolLight,   note: 'Submitted answers + documents' },
    { label: 'Diagnostic report done', value: stats.diagnostics.complete,   colour: S.gold, note: 'Got their personalised diagnosis' },
    { label: 'Documents generated',    value: stats.generations.total,      colour: S.petrol,  note: 'Total docs created (not unique users)' },
    { label: 'Job analyses run',       value: stats.analyses.total,         colour: S.success,  note: 'Ran at least one match analysis' },
    { label: 'Converted to paid',      value: stats.users.paid,             colour: S.gold, note: 'Active paid subscription' },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        border: `1px solid rgba(45,90,110,0.18)`, borderRadius: 14,
        padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap',
      }}>
        <div>
          <p style={{ margin: '0 0 4px', fontSize: 10, fontWeight: 900, letterSpacing: '0.12em', textTransform: 'uppercase', color: S.petrol }}>
            Vercel Analytics
          </p>
        ...
        <a
          ...
          style={{
            ...
            background: `${S.petrol}18`, border: `1px solid ${S.petrol}40`,
            color: S.petrol, borderRadius: 10, padding: '9px 16px',
      ...
      {/* Conversion funnel */}
      <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '22px 24px' }}>
          ...
          {stages.map((stage, i) => {
            ...
            return (
              <div key={stage.label}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                    {dropPct !== null && (
                      <span style={{
                        ...
                        background: dropPct >= 70 ? `${S.success}18` : dropPct >= 40 ? `${S.gold}18` : `${S.danger}18`,
                        color: dropPct >= 70 ? S.success : dropPct >= 40 ? S.gold : S.danger,
                      }}>
                        {dropPct}% from prev
                      </span>
                    )}
                ...
                <div style={{ height: 8, borderRadius: 99, background: warm.colors.bgAlt, overflow: 'hidden' }}>
                  <div style={{
        ...
        {/* Summary row */}
        <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${warm.colors.borderWhisper}`, display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          <div>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim }}>Onboarding rate</p>
            <p style={{ margin: '3px 0 0', fontSize: 22, fontWeight: 900, color: S.petrol }}>{pct(stats.users.onboarded, stats.users.total)}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim }}>Diagnostic rate</p>
            <p style={{ margin: '3px 0 0', fontSize: 22, fontWeight: 900, color: S.petrolLight }}>{pct(stats.diagnostics.complete, stats.users.onboarded)}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim }}>Paid conversion</p>
            <p style={{ margin: '3px 0 0', fontSize: 22, fontWeight: 900, color: S.gold }}>{pct(stats.users.paid, stats.users.total)}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: S.dim }}>Docs per user</p>
            <p style={{ margin: '3px 0 0', fontSize: 22, fontWeight: 900, color: S.gold }}>
...
function MiniBar({ value, max, colour }: { value: number; max: number; colour: string }) {
  return (
    <div style={{ flex: 1, height: 4, borderRadius: 99, background: warm.colors.bgAlt, overflow: 'hidden' }}>
...
function BehaviourTab() {
  ...
  if (isError || !data) return (
    <p style={{ color: S.danger, fontSize: 14 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'stretch' }}>
        <div style={{
          background: `${S.gold}10`, border: `1px solid ${S.gold}28`, borderRadius: 14,
          ...
          <a href="https://us.posthog.com/project/413547" target="_blank" rel="noopener noreferrer"
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 7,
              background: `${S.gold}20`, border: `1px solid ${S.gold}40`,
        ...
        <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 28px', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        ...
        <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
          ...
          <Breakdown items={data.docTypes} colour={S.petrol} empty="No generation events yet" max={maxDoc} />
        </div>
        <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
          <SectionHead label="Features opened (30d)" />
          <Breakdown items={data.features} colour={S.petrolLight} empty="No feature events yet" max={maxFeature} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: warm.colors.bgSurface, border: `1px solid ${warm.colors.borderWhisper}`, borderRadius: 14, padding: '18px 20px' }}>
          ...
          <Breakdown items={data.cancelReasons} colour={S.danger} empty="None recorded yet" max={maxCancel} />
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <Zap size={16} style={{ color: S.petrol }} />
              <span style={{ fontSize: 11, fontWeight: 900, letterSpacing: '0.12em', textTransform: 'uppercase', color: S.petrol }}>
                Admin
              </span>
            </div>
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 900, letterSpacing: '-0.02em', color: S.main }}>Dashboard</h1>
          ...
          {([['overview', 'Overview', BarChart2], ['funnel', 'Funnel', TrendingUp], ['behaviour', 'Behaviour', Activity], ['friday', 'Friday Brief', ClipboardList]] as const).map(([key, label, Icon]) => (
            <button key={key} onClick={() => setTab(key)}
              style={{
                ...
                background: tab === key ? warm.colors.bgSurface : 'transparent',
        ...
        {(tab === 'overview' || tab === 'funnel') && (
          isLoading ? (
            ...
            <p style={{ color: S.danger }}>Failed to load stats. Make sure you're logged in as admin.</p>
          ) : tab === 'overview' ? (
